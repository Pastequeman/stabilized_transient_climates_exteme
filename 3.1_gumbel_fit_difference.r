...
# 5) fit gumbel to both series
# 6) investigate variation
# 7) 
...
#### Gumbel fitting
## compute gumbel for both 30-y samples
## take diff in scale and location
## using permutation/bootstrap assess significant of difference
for (i in 1:4) {
  # carefull. low flow have to sort -x !
  if (i %in% c(1, 2)) {
    sample1 <- apply(outputs[[i]], 2, function(x) sort(x, na.last = TRUE))
    sample2 <- apply(outputs[[(i+4)]], 2, function(x) sort(x, na.last = TRUE))    
  } else {
    sample1 <- outputs[[i]] * -1
    sample2 <- outputs[[(i+4)]] * -1
    sample1 <- apply(sample1, 2, function(x) sort(x, decreasing = TRUE, na.last = TRUE))
    sample2 <- apply(sample2, 2, function(x) sort(x, decreasing = TRUE, na.last = TRUE))
  }

  ## fit L-moment for both samples
  sample1_M1 <- apply(sample1, 2, function(x) if (sum(is.na(x)) >= 25) {NA} else {mean(x, na.rm = TRUE)})
  sample1_M2 <- apply(sample1, 2, function(x) if (sum(is.na(x)) >= 25) {NA} else {1 / 30 * sum((seq(1, 30, 1) - 1) / (30 - 1) * x)})
  sample1_L1 <- sample1_M1
  sample1_L2 <- 2 * sample1_M2 - sample1_M1

  sample2_M1 <- apply(sample2, 2, function(x) if (sum(is.na(x)) >= 25) {NA} else {mean(x, na.rm = TRUE)})
  sample2_M2 <- apply(sample2, 2, function(x) if (sum(is.na(x)) >= 25) {NA} else {1 / 30 * sum((seq(1, 30, 1) - 1) / (30 - 1) * x)})
  sample2_L1 <- sample2_M1
  sample2_L2 <- 2 * sample2_M2 - sample2_M1
  
  sample1_alpha      <- sample1_L2 / log(2)
  sample1_epsilon    <- sample1_L1 - sample1_alpha * 0.57721
  sample1_Y100_ext   <- sample1_epsilon - sample1_alpha * log(-log(1 - 1/100))

  sample2_alpha      <- sample2_L2 / log(2)
  sample2_epsilon    <- sample2_L1 - sample2_alpha * 0.57721
  sample2_Y100_ext   <- sample2_epsilon - sample2_alpha * log(-log(1 - 1/100))

  ## save the basic fit
  write_csv(data.frame(alpha = sample1_alpha, espilon = sample1_epsilon, y100 = sample1_Y100_ext),
            paste0("/data01/julien/projects/extreme_trans_stab/OUT/gumbel/", casefold(MODEL), "_", GCM, "_",
                   ...
                   out_var[i], ".csv"))
  write_csv(data.frame(alpha = sample2_alpha, espilon = sample2_epsilon, y100 = sample2_Y100_ext),
            paste0("/data01/julien/projects/extreme_trans_stab/OUT/gumbel/", casefold(MODEL), "_", GCM, "_",
                   EXPERIMENTS[2], "_", as.character(PERIOD_2[1]), "-", as.character(PERIOD_2[2]), "_",
                   out_var[(i+4)], ".csv"))

  ## now the diff and permutation test
  if (PERMUTATION == "yes") {
    # original diff:
    diff_espsilon <- sample1_epsilon - sample2_epsilon
    diff_alpha    <- sample1_alpha - sample2_alpha
    diff_y100     <- sample1_Y100_ext - sample2_Y100_ext
    # estimates
    pooled            <- rbind(sample1, sample2)
    estimates_alpha   <- matrix(nrow = perm_it, ncol = 259200)
    ...
    for (k in 1:perm_it) {
      ...
      # sort first!
      if (i %in% c(1, 2)) {
        sample1 <- apply(pooled[picked,], 2, function(x) sort(x, na.last = TRUE))
        sample2 <- apply(pooled[-picked,], 2, function(x) sort(x, na.last = TRUE))    
      } else {
        sample1 <- apply(sample1, 2, function(x) sort(x, decreasing = TRUE, na.last = TRUE))
        sample2 <- apply(sample2, 2, function(x) sort(x, decreasing = TRUE, na.last = TRUE))
      }
      ...
      # save
      estimates_alpha[k,]   <- sample1_L2 / log(2) - sample2_L2 / log(2) 
      estimates_epsilon[k,] <- (sample1_L1 - sample1_alpha * 0.57721) - (sample2_L1 - sample2_alpha * 0.57721)
      estimates_y100[k,]    <- ((sample1_L1 - sample1_alpha * 0.57721) - (sample1_L2 / log(2)) * log(-log(1 - 1/100))) -
                              ((sample2_L1 - sample2_alpha * 0.57721) - (sample2_L2 / log(2)) * log(-log(1 - 1/100)))
      
    } # end permuation iteration loop
    to_save     <- data.frame(low_alpha   = apply(estimates_alpha, 2, function(x) quantile(x, 0.025, na.rm = TRUE)),
                              hgt_alpha   = apply(estimates_alpha, 2, function(x) quantile(x, 0.975, na.rm = TRUE)),
                              low_epsilon = apply(estimates_epsilon, 2, function(x) quantile(x, 0.025, na.rm = TRUE)),
                              hgt_epsilon = apply(estimates_epsilon, 2, function(x) quantile(x, 0.975, na.rm = TRUE)),
                              low_y100   = apply(estimates_y100, 2, function(x) quantile(x, 0.025, na.rm = TRUE)),
                              hgt_y100   = apply(estimates_y100, 2, function(x) quantile(x, 0.975, na.rm = TRUE)))
    write_csv(to_save, paste0("/data01/julien/projects/extreme_trans_stab/OUT/tail_analysis/", casefold(MODEL), "_", GCM, "_",
                              EXPERIMENTS[1], "_", as.character(PERIOD_1[1]), "-", as.character(PERIOD_1[2]), "_",
                              EXPERIMENTS[2], "_", as.character(PERIOD_2[1]), "-", as.character(PERIOD_2[2]), "_",
                              out_var[(i)], ".csv"))
  } # end permuation test
